<?php
    ...
    $status = $_GET['status'];
    switch($status){
        case 'ongoing': $status = 'on-going';
            break;
        case 'onhold': $status = 'on hold';
            break;
...
if ($user_role === 'admin') {
  //$allTicket = $ticket::findByStatus('New');
  if($project===''){
    $allTicket = $ticket::findBy("WHERE UPPER(status)=UPPER('". $status . "')" );
  }
  else{
    $allTicket = $ticket::findBy("WHERE UPPER(status)=UPPER('". $status . "') AND UPPER(project)=UPPER('". $project ."')" );
  }
  
  $sql = "SELECT * FROM ticket ORDER BY id DESC";
  $stmt = $db->prepare($sql);
} else {
  $allTicket = $ticket::findByStatusAndTeam('New', $user_team_id);
                ...
                <div class="table-responsive">
                    ...
                    <?php foreach($allTicket as $ticket): ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo date('m-d-Y', strtotime($ticket->schedule_date)) ?></td>
                                <td><button class="btn btn-danger"><?php echo $ticket->status ?></button></td>
                                <td><?php echo $ticket->priority ?></td>
                                <?php $createddate = new DateTime($ticket->created_at)?>
                                <td><?php echo $createddate->format('m-d-Y H:i:s')?> </td>
                                <?php $lastmodified = new DateTime($ticket->date_modified)?>
                                <td><?php echo $lastmodified->format('m-d-Y H:i:s')?> </td>
                            </tr>
                        <?php endforeach?>
                    </tbody>
    ...
    function applyFilters() {
        ...
        table.querySelectorAll("tbody tr").forEach(row => {
            ...
            Object.keys(activeFilters).forEach(columnIndex => {
                let cellValue = row.cells[columnIndex].textContent.trim();
                alert(activeFilters[columnIndex].length);
